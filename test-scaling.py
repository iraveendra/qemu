...
ADMIN_PASSWORD = 'SAp0sNwwvXImcI3guDCY74xGJLkzZAVXWm8GNF7IJh6jEPqbzqhms4Tiat28yJL'
DEFAULT_RATE = 10
DEFAULT_DURATION = 60
MAX_REPLICAS = 10

def send_request(input_data):
    url = f"http://{ADMIN_USERNAME}:{ADMIN_PASSWORD}@10.62.0.13:8080/function/my-chatbot"
    response = requests.post(url, data=input_data)
    return response

def get_replica_count():
    url = f"http://{ADMIN_USERNAME}:{ADMIN_PASSWORD}@10.62.0.13:8080/system/functions"
...
def scale_replicas(replicas):
    url = f"http://{ADMIN_USERNAME}:{ADMIN_PASSWORD}@10.62.0.13:8080/system/scale-function/my-chatbot"
